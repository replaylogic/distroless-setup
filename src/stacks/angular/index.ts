...
import { Plan, camelToEnv, exists, isDir, loadJson, readText, rel, tryJson, walkFiles } from "../../core/files";
...
import { GO_IMAGE, RUNTIME_IMAGE, RuntimeConfig, askPort, askServerDir, gatherHeaders, planServer, scanNginx } from "../shared/static-spa";
import { FILE_PREFIX, renderDockerfile, renderTsModel, renderTsService } from "./templates";
...
export const angularStack: Stack = {
  ...
  async run(ctx: Ctx): Promise<StackResult> {
    ...
    const serverFiles = planServer(plan, repo, serverDir, rc, headers);
    ...
    const needed = ["package.json", "angular.json", ...(b.install.pm.lock ? [b.install.pm.lock] : []), ...b.install.extras.map((e) => e.replace(/\/+$/, "")),
      ...serverFiles, ...(rc.enabled && rc.override ? [rc.override] : []),
